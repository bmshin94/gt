...
const CIRCLE_X =
  '<svg class="error" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>';

const PAGES = {
  success: {
    title: 'Signed in',
    icon: CIRCLE_CHECK,
    body: "You're signed in to General Translation. You can close this window and return to your terminal.",
  },
  error: {
    title: "Sign in didn't complete",
    icon: CIRCLE_X,
    body: 'Return to your terminal for details. You can close this window.',
  },
};

export function renderCallbackPage(succeeded: boolean): string {
  const page = PAGES[succeeded ? 'success' : 'error'];
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
